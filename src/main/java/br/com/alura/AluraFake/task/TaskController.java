...
public class TaskController {
    ...
    public ResponseEntity<CreatedTask> taskNewOpentextPost(@RequestBody @Valid OpenTextTask openTextAnswer) {
        return ResponseEntity.status(201).body(this.taskService.createNewTask(openTextAnswer, TaskType.OPEN_TEXT));
    }

    @Transactional
    @PostMapping("/task/new/singlechoice")
    public ResponseEntity<CreatedTask> taskNewSinglechoicePost(@RequestBody @Valid SingleChoiceTask singleChoiceTask) {
